...
async function main() {
  ...
  // Criar usuário MANAGER (responsável) de teste
  const cong1 = await prisma.congregation.findFirst({
    where: { code: 'A1-01' }
  });

  await prisma.user.upsert({
    where: { whatsapp: '5511999999999' },
    update: { congregationId: cong1.id, role: 'MANAGER', active: true },
    create: {
      name: 'Responsável Teste',
      whatsapp: '5511999999999',
      congregationId: cong1.id,
      role: 'MANAGER',
      active: true
    }
  });
